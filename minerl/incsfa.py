from expansion import quadratic_expansion
...
class IncSFA2(IncSFA):
    ...
    **Instance variables of interest**
    ...
    def __init__(self, input_dim, whitening_output_dim, output_dim, eps=0.001, **kwargs):
        super().__init__(input_dim * input_dim + input_dim, whitening_output_dim, output_dim, eps, **kwargs)
        self.unexpanded_input_dim = input_dim

    def update(self, x, newEpisode=False):
        assert x.shape[1] == self.unexpanded_input_dim
        return super().update(quadratic_expansion(x), newEpisode)

    def execute(self, x):
        assert x.shape[1] == self.unexpanded_input_dim
        return super().execute(quadratic_expansion(x))
