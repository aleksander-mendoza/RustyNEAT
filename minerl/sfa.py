...
class SFA(TransformerMixin, BaseEstimator):
    ...
    def _accumulate(self, X):
        ...
        if self._n_partial_diff_samples == 0:
            ...
            self.outer_diff_sum += np.dot(diff_X.T,
                                          diff_X)  # + np.dot(last_partial_diff[:, None], last_partial_diff[None, :])
    ...
    def _fit_generalized_eigenmethod(self):
        ...
        matrices is solved.
        """
        C = (self.outer_sum - np.outer(self.sample_sum, self.sample_sum) / (self._n_partial_samples)) / (
                    self._n_partial_samples - 1)
    ...
    def transform(self, X):
        ...
        if (self.is_fitted) and (not self.is_partially_fitted):
            ...
            if not (self.is_fitted):
                ...
                self.is_fitted_ = True
            y = np.dot(X, self._partial_eigenvectors[:, :self.n_components]) - np.dot(
                self.sample_sum / (self._n_partial_samples), self._partial_eigenvectors[:, :self.n_components])
        else:
    ...
    def affine_parameters(self):
        ...
        if n_missing_components > 0 and self.fill_mode not in (None, "zero"):
            raise RuntimeError(
                "Affine parameters cannot be extracted: missing output dimensions are filled with something else then a 0-signal. Please set 'fill_mode' to 'zero' to avoid this or make sure that your input data has high enough rank.")
        W_whiten = (self.pca_whiten_.components_.T / np.sqrt(self.pca_whiten_.explained_variance_)).T[
            self.nontrivial_indices_]
