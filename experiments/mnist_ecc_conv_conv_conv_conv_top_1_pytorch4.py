...
layer1_save_file = 'mnist_ecc_conv_conv_conv_conv_top_1_pytorch4 layer1.model'
layer2_save_file = 'mnist_ecc_conv_conv_conv_conv_top_1_pytorch4 layer2.model'
layer3_save_file = 'mnist_ecc_conv_conv_conv_conv_top_1_pytorch4 layer3.model'
layer4_save_file = 'mnist_ecc_conv_conv_conv_conv_top_1_pytorch4 layer4.model'
...
def pretrain(base, head, w, h, iterations=1000000, interval=100000, test_patches=20000):
    ...
    assert base is None or base.out_shape == head.in_shape
...
if os.path.exists(layer3_save_file):
    ...
    l3_col = ecc.CpuEccDense(output=[1, 1], kernel=[5, 5], stride=[1, 1], in_channels=l2_col.out_channels,
                             out_channels=100, k=4)
    l3_col.top1_per_region = True
    l3_col.dropout_per_kernel(0.3)
    l2 = l2_col.repeat_column(l3_col.in_shape[:2])
    l1 = l1_col.repeat_column(l2.in_shape[:2]).to_machine()
    l1.push(l2)
    pretrain(l1, l3_col, 10, 10)
...
if os.path.exists(layer4_save_file):
    ...
    l4_col = ecc.CpuEccDense(output=[1, 1], kernel=[5, 5], stride=[1, 1], in_channels=l3_col.out_channels,
                             out_channels=15*15, k=9)
    l4_col.top1_per_region = True
    l4_col.dropout_per_kernel(0.3)
...
train_mnist = MNIST[:SPLIT_POINT]
train_labels = LABELS[:SPLIT_POINT]
train_d = D(train_mnist, train_labels)

eval_mnist = MNIST[SPLIT_POINT:END]
eval_labels = LABELS[SPLIT_POINT:END]
