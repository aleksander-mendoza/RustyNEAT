...
def run(learn=False, log=False):
    ...
    return apical_feedback, cast_votes, updated_votes, layer1
...
def train():
    ...
    for _ in range(1000):
        apical_feedback_l, cast_votes_l, updated_votes_l, layer1_l = run(learn=True)
        move_agent(1, 0)
        apical_feedback_r, cast_votes_r, updated_votes_r, layer1_r = run(learn=True)
        move_agent(-1, 0)
        ...
        plt.plot([layer1_column_activity_max_overlap]*len(layer1_column_activity_overlaps), label="max overlap possible")
        plt.legend()
        plt.pause(0.01)
    plt.show()
