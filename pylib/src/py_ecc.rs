...
use htm::{VectorFieldOne, Idx, EccLayer, Rand, SDR, w_idx, ConvShape, Shape2, ConvWeightVec, HasShape, ConvShapeTrait, HasShapeMut, Metric, MetricL1, MetricL2};
...
pub struct CpuEccDense {
    pub(crate) ecc: htm::CpuEccDense<f32, MetricL1<f32>>,
}
...
pub struct CpuEccMachine {
    pub(crate) ecc: htm::CpuEccMachine<f32, MetricL1<f32>>,
}
...
///
#[pyclass]
pub struct CpuEccDenseL2 {
    pub(crate) ecc: htm::CpuEccDense<f32, MetricL2<f32>>,
}
...
///
#[pyclass]
pub struct CpuEccMachineL2 {
    pub(crate) ecc: htm::CpuEccMachine<f32, MetricL2<f32>>,
}


#[pyproto]
impl PySequenceProtocol for CpuEccMachineL2 {
...
#[pyproto]
impl PySequenceProtocol for CpuEccDenseL2 {
    fn __len__(&self) -> usize {
        self.ecc.len()
    }
}
...
impl CpuEccMachineL2 {
    ...
    #[text_signature = "(layer_idx)"]
    pub fn get_layer(&self, layer:usize) -> CpuEccDenseL2 {
        CpuEccDenseL2{ecc:self.ecc[layer].clone()}
    }
    #[text_signature = "(layer_idx,layer_instance)"]
    pub fn set_layer(&mut self, layer:usize, net:&CpuEccDenseL2)  {
        assert_eq!(self.ecc[layer].in_shape(),net.ecc.in_shape(),"Input shapes don't match");
        assert_eq!(self.ecc[layer].out_shape(),net.ecc.out_shape(),"Output shapes don't match");
        self.ecc[layer] = net.ecc.clone()
    }
    ...
    #[text_signature = "()"]
    pub fn pop(&mut self) -> Option<CpuEccDenseL2> {
        self.ecc.pop().map(|ecc|CpuEccDenseL2 { ecc })
    }

    #[text_signature = "()"]
    pub fn pop_front(&mut self) -> Option<CpuEccDenseL2> {
        self.ecc.pop_front().map(|ecc|CpuEccDenseL2 { ecc })
    }

    #[text_signature = "(top_layer)"]
    pub fn push(&mut self, top: &CpuEccDenseL2) {
        self.ecc.push(top.ecc.clone())
    }
    #[text_signature = "(bottom_layer)"]
    pub fn prepend(&mut self, bottom: &CpuEccDenseL2) {
        self.ecc.prepend(bottom.ecc.clone())
    }
    #[text_signature = "(top_layer,column_pos)"]
    pub fn push_repeated_column(&mut self, top: &CpuEccDenseL2,column_pos: Option<[Idx; 2]>) {
        self.ecc.push_repeated_column(&top.ecc,column_pos.unwrap_or([0,0]))
    }
    #[text_signature = "(bottom_layer,column_pos)"]
    pub fn prepend_repeated_column(&mut self, bottom: &CpuEccDenseL2,column_pos: Option<[Idx; 2]>) {
        self.ecc.prepend_repeated_column(&bottom.ecc,column_pos.unwrap_or([0,0]))
    }
    #[text_signature = "(layer)"]
    pub fn set_initial_activity(&mut self, layer: usize) {
...
impl_save_load!(CpuEccMachineL2,ecc);
impl_save_load!(CpuEccDenseL2,ecc);
...
impl CpuEccDenseL2 {
    ...
    #[text_signature = "()"]
    pub fn to_machine(&self) -> CpuEccMachineL2 {
        CpuEccMachineL2 { ecc: self.ecc.clone().into_machine() }
    }
    #[text_signature = "()"]
    pub fn restore_dropped_out_weights(&mut self) {
        self.ecc.restore_dropped_out_weights()
    ...
    pub fn new(output: PyObject, kernel: PyObject, stride: PyObject, in_channels: Idx, out_channels: Idx, k: Idx) -> PyResult<Self> {
        ...
        let kernel = arr2(py, &kernel)?;
        let stride = arr2(py, &stride)?;
        Ok(Self{ ecc: htm::CpuEccDense::new(ConvShape::new(output, kernel, stride, in_channels, out_channels), k, &mut rand::thread_rng()) })
    }
    #[staticmethod]
    #[text_signature = "(input, kernel, stride,in_channels, out_channels)"]
    pub fn new_in(input: PyObject, kernel: PyObject, stride: PyObject, in_channels: Idx, out_channels: Idx, k: Idx) -> PyResult<Self> {
        ...
        let kernel = arr2(py, &kernel)?;
        let stride = arr2(py, &stride)?;
        Ok(Self { ecc: htm::CpuEccDense::new(ConvShape::new_in(input.add_channels(in_channels), out_channels,kernel, stride), k, &mut rand::thread_rng()) })
    }
    #[text_signature = "(new_stride)"]
    pub fn set_stride(&mut self, new_stride: PyObject) -> PyResult<()> {
        let gil = Python::acquire_gil();
