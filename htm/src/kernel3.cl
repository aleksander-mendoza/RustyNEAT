...
typedef struct _HtmFeedforwardConnection3{
    uint minicolumn_id;
    float permanence;
    uint input_id;
    bool is_inhibitory;
} HtmFeedforwardConnection3;
typedef struct _HtmInput3{
    uint connection_offset;
    uint connection_len;
    bool is_active;
} HtmInput3;
typedef struct _HtmMinicolumn3{
...
__kernel void htm_calculate_overlap(
                  ...
                  __global HtmMinicolumn3 * minicolumns,
                  __global HtmInput3 * inputs,
                  __global HtmFeedforwardConnection3 * feedforward_connections
){
    const size_t input_idx = get_global_id(0);
    const uint input_neuron_idx = sdr_input[input_idx];
    const HtmInput3 input_neuron = inputs[input_neuron_idx];
...
__kernel void htm_clean_up_active_inputs(__global uint * sdr_input,__global HtmInput3 * inputs){
    const size_t input_idx = get_global_id(0);
    const uint input_neuron_idx = sdr_input[input_idx];
