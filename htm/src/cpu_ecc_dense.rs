...
impl<M:Metric<D>> CpuEccDense<M> {
    ...
    pub fn weights(&self) -> &ConvWeights<M> {
        &self.w
    }
    pub fn population(&self) -> &CpuEccPopulation<M> {
        &self.p
    }
    pub fn weights_mut(&mut self) -> &mut ConvWeights<M> {
        &mut self.w
    }
    ...
    pub fn from(weights: ConvWeights<M>, pop: CpuEccPopulation<M>) -> Self {
...
mod tests {
    ...
    fn test9_<M:Metric<D>>() -> Result<(), String> {
        ...
        for i in 0..1024 {
            ...
            input.normalize();
            assert_ne!(input.len(), 0);
            assert!(a.weights().weight_slice().iter().zip(a2.weights().weight_slice().iter()).all(|(a, b)| a==b), "w {:?}!={:?}", a.weight_slice(), a2.weight_slice());
    ...
    fn test10_<M:Metric<D>>() -> Result<(), String> {
        ...
        for i in 0..1024 {
            ...
            input.normalize();
            assert_ne!(input.len(), 0);
            assert!(a.weights().weight_slice().iter().zip(a2.weights().weight_slice().iter()).all(|(a, b)| a==b), "w {:?}!={:?}", a.weights().weight_slice(), a2.weights().weight_slice());
            assert!(a.population().sums.iter().zip(a2.population().sums.iter()).all(|(a, b)| a==b), "sums {:?}!={:?}", a.population().sums, a2.population().sums);
            assert!(a.population().activity.iter().zip(a2.population().activity.iter()).all(|(a, b)| a==b), "activity {:?}!={:?}", a.population().activity, a2.population().activity);
            let mut o = a.run(&input);
            let mut o2 = a2.run(&input);
