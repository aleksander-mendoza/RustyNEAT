...
def rand_patch(patch_size):
    ...
    img = MNIST[int(numpy.random.rand() * len(MNIST))]
...
def normalise_img(x):
    x = (x / 255 > 0.8).astype(numpy.float32)
    return x
...
def sparse_weights(input_size, output_size, sparsity, k):
    pop = htm.Population(output_size)
    pop.add_uniform_rand_inputs_from_range((0, input_size), sparsity)
    m = htm.CpuHTM(input_size, k, pop)
    return m


def dense_weights(input_size, output_size):
    s = np.random.rand(input_size, output_size)
    w = nd.from_numpy(s, context=c)
    w /= s.sum(0)
    return w


class Project:
    def __init__(self, input_size, output_size, sparsity=None, threshold=0.2, plasticity=0.0001, k=1):
...
class Convolve:
    def __init__(self, input_size, kernel, stride, in_channels, out_channels, k=1, connection_sparsity=None):
        ...
        self.columns: [Project] = [[Project(self.in_column_shape, self.out_column_shape,
                                            k=k, sparsity=connection_sparsity)
