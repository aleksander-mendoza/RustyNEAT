...
layer1_save_file = 'mnist_ecc_conv_conv_pytorch layer1.model'
layer2_save_file = 'mnist_ecc_conv_conv_pytorch layer2.model'
...
if os.path.exists(layer2_save_file):
    ...
    l2_col = ecc.CpuEccDense(output=[1, 1], kernel=[5, 5], stride=[1, 1], in_channels=l1_col.out_channels, out_channels=81, k=1)
    if PRETRAIN:
        l1 = l1_col.repeat_column([5, 5]).to_machine()
        pretrain(l1, l2_col, 9, 9)
...
linear = torch.nn.Linear(l2.out_volume, 10)
...
# results if PRETRAIN==True
#    train= 0.95325 eval= 0.9729
#    train= 0.993625 eval= 0.9734
